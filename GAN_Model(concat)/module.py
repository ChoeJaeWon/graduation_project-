...
#File name
FILEX_SPEED = '../Data/Speed/x_concat_data_2016204_5min_60min_60min_only_speed.csv' #speed만 잘라낸 파일 이름(X data)
...
ADV_FC_OS_ALLTEST = [79+OPTIMIZED_EPOCH_FC, 35+OPTIMIZED_EPOCH_FC, 71+OPTIMIZED_EPOCH_FC, 55+OPTIMIZED_EPOCH_FC, 85+OPTIMIZED_EPOCH_FC]
ADV_CONV_OS_ALLTEST = [41+OPTIMIZED_EPOCH_CONV, 55+OPTIMIZED_EPOCH_CONV, 45+OPTIMIZED_EPOCH_CONV, 32+OPTIMIZED_EPOCH_CONV, 42+OPTIMIZED_EPOCH_CONV]
ADV_LSTM_OS_ALLTEST = [73, 57, 42, 52]#4CV까지 돌았
ADV_CONVLSTM_OS_ALLTEST = [41+OPTIMIZED_EPOCH_CONV_LSTM, 37+OPTIMIZED_EPOCH_CONV_LSTM, 66+OPTIMIZED_EPOCH_CONV_LSTM, 55+OPTIMIZED_EPOCH_CONV_LSTM, 55+OPTIMIZED_EPOCH_CONV_LSTM]
...
ADV_FC_EXO_ALLTEST = [32+OPTIMIZED_EPOCH_FC, 57+OPTIMIZED_EPOCH_FC, 36+OPTIMIZED_EPOCH_FC, 32+OPTIMIZED_EPOCH_FC, 32+OPTIMIZED_EPOCH_FC]
ADV_CONV_EXO_ALLTEST = [33+OPTIMIZED_EPOCH_CONV, 39+OPTIMIZED_EPOCH_CONV, 35+OPTIMIZED_EPOCH_CONV, 32+OPTIMIZED_EPOCH_CONV,	32+OPTIMIZED_EPOCH_CONV]
ADV_LSTM_EXO_ALLTEST = [41+OPTIMIZED_EPOCH_LSTM, 83+OPTIMIZED_EPOCH_LSTM, 41+OPTIMIZED_EPOCH_LSTM, 41+OPTIMIZED_EPOCH_LSTM,	41+OPTIMIZED_EPOCH_LSTM]
...
VECTOR_SIZE = 143 #fc와 lstm에 들어가는 vector의 크기 [default 83]
TIME_STAMP = 60 #lstm과 fc의 vector에서 고려해주는 시간 [default 12]
