...
def model(S, E, Y, DISCRIMINATOR_BA,  DISCRIMINATOR_DR):
    ...
    DE = tf.concat([S, E], axis=1)

    loss_D = -tf.reduce_mean(tf.log(Discriminator_model(adv_y, DE, DISCRIMINATOR_BA, DISCRIMINATOR_DR)) + tf.log(1 - Discriminator_model(adv_g, DE, DISCRIMINATOR_BA, DISCRIMINATOR_DR)))
    loss_G = -tf.reduce_mean(tf.log(Discriminator_model(adv_g, DE, DISCRIMINATOR_BA, DISCRIMINATOR_DR))) + DISCRIMINATOR_ALPHA * cost_MSE  # MSE 는 0~ t까지 있어봤자 같은 값이다.
