...
_result_dir = RESULT_DIR + "CV" + str(CROSS_ITERATION_NUM) + "/" + "ADV_CONV_PHASE_FREE"
cr_idx = 0
OS_OR_EXO = True
if FILEX_EXO.find("Zero") < 0:
    OS_OR_EXO = False
for train_idx, test_idx in load_Data():
    ...
    if RESTORE_GENERATOR_FLAG:
        ...
        last_epoch = tf.Variable(61, name=LAST_EPOCH_NAME)
    init()
    sess = tf.Session()
    ...
    if FILEX_EXO.find("Zero") >= 0:
        CURRENT_POINT_DIR = CHECK_POINT_DIR + "ADV_CONV_OS_PHASE_FREE_" + str(cr_idx) + "/"
        writer_train = tf.summary.FileWriter("./tensorboard/adv_conv_os_phase_free/train%d" % cr_idx, sess.graph)
        writer_test = tf.summary.FileWriter("./tensorboard/adv_conv_os_phase_free/test%d" % cr_idx, sess.graph)
    else:
        CURRENT_POINT_DIR = CHECK_POINT_DIR + "ADV_CONV_EXO_PHASE_FREE_" + str(cr_idx) + "/"
        writer_train = tf.summary.FileWriter("./tensorboard/adv_conv_exo_phase_free/train%d" % cr_idx, sess.graph)
        writer_test = tf.summary.FileWriter("./tensorboard/adv_conv_exo_phase_free/test%d" % cr_idx, sess.graph)
    ...
    output_data(train_result, test_result, 'adv_conv_phase_free'+ "_" + str(DISCRIMINATOR_LAYER_NUM) + "_" + str(LEARNING_RATE)[2:]+ "_" + format(DISCRIMINATOR_ALPHA,'f')[2:] + "_" + str(PHASE1_EPOCH) + "_"+ str(PHASE2_EPOCH) +"_"+ str(TRAIN_NUM)+ "_" , cr_idx, _result_dir)
...
output_result(final_result, 'adv_conv_phase_free' + "_" + str(DISCRIMINATOR_LAYER_NUM) + "_" + str(LEARNING_RATE)[2:]+"_" + format(DISCRIMINATOR_ALPHA, 'f')[2:] + "_" + str(PHASE1_EPOCH) + "_"+ str(PHASE2_EPOCH)+"_"+ str(TRAIN_NUM)+ "_", cr_idx, _result_dir)