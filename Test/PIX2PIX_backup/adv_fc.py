...
def model_base(S, E, Y, BA, DR, DISCRIMINATOR_BA, DISCRIMINATOR_DR):
    ...
    with tf.control_dependencies(D_update_ops):
        train_D = tf.train.AdamOptimizer(learning_rate=ADV_LEARNING_RATE).minimize(loss_D, var_list=[vars_D, discriminator_weights]) #이 부분은 모델별로 고정

    G_update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='generator_fc')
    with tf.control_dependencies(G_update_ops):
        train_G = tf.train.AdamOptimizer(learning_rate=ADV_LEARNING_RATE).minimize(loss_G, var_list=[vars_G ,fc_weights])
    ''''''
    ...
    with tf.control_dependencies(G_MSE_update_ops):
        train_G_MSE = tf.train.AdamOptimizer(learning_rate=ADV_LEARNING_RATE).minimize(loss_G_MSE, var_list=[vars_G, fc_weights])
    ''''''
    ''''''
    ...
    with tf.control_dependencies(G_Gen_update_ops):
        train_G_Gen = tf.train.AdamOptimizer(learning_rate=ADV_LEARNING_RATE).minimize(loss_G_Gen,var_list=[vars_G, fc_weights])
    ''''''
...
def train(S_data, E_data, Y_data, cost_MAE, cost_MSE, cost_MAPE, cost_MAE_hist, cost_MSE_hist, cost_MAPE_hist, train_D, train_G, train_idx, test_idx, cr_idx,  writer_train, writer_test, train_result, test_result, CURRENT_POINT_DIR, start_from):
    ...
    for tr_idx in range(start_from, TRAIN_NUM):
        ...
        for ba_idx in range(BATCH_NUM):
            ...
            '''
            if (tr_idx % (PHASE1_EPOCH+PHASE2_EPOCH)) < PHASE2_EPOCH:  # generator 먼저 선학습 후 discriminator 단독 학습
                _ = sess.run([train_D],
                             feed_dict={S: S_train, E: E_train, Y: Y_train, BA: True, DR: FC_TR_KEEP_PROB, DISCRIMINATOR_BA: True,
                                        DISCRIMINATOR_DR: DISCRIMINATOR_TR_KEEP_PROB})
                # print(sess.run(vars_G[2]))
...
for train_idx, test_idx in Week_CrossValidation():
    ...
    if RESTORE_GENERATOR_FLAG:
        last_epoch = tf.Variable(OPTIMIZED_EPOCH_FC, name=LAST_EPOCH_NAME) #받아올 방법이 없네..
    else:
