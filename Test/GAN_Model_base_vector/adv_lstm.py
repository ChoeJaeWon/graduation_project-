...
def model_base(S, E, Y, DISCRIMINATOR_BA,  DISCRIMINATOR_DR):
    for gen_idx in range(GEN_NUM):
        if gen_idx == 0:
            layer = tf.reshape(LSTM_model(S[gen_idx], E[gen_idx]), [1, BATCH_SIZE])
        else:
            layer = tf.concat([layer, tf.reshape(LSTM_model(S[gen_idx], E[gen_idx]), [1, BATCH_SIZE])], axis=0)
    ...
    loss_G = -tf.reduce_mean(tf.log(Discriminator_model(layer, DE, DISCRIMINATOR_BA, DISCRIMINATOR_DR, True))) + DISCRIMINATOR_ALPHA * cost_MSE  # MSE 는 0~ t까지 있어봤자 같은 값이다.
