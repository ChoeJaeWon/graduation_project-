...
#training 해준다.
def train(S_data, C_data, E_data, Y_data, cost_MAE, cost_MSE, cost_MAPE,  cost_MAE_hist, cost_MSE_hist, cost_MAPE_hist, optimal, train_idx, test_idx, cr_idx, writer_train, writer_test, train_result, test_result, CURRENT_POINT_DIR, start_from):
    ...
    for tr_idx in range(start_from, TRAIN_NUM):
        ...
        for ba_idx in range(BATCH_NUM):
            ...
            CY_train = batch_slice(S_data, train_idx+12, ba_idx, 'LSTM', 1)
            Y_train = batch_slice(Y_data, train_idx, ba_idx, 'LSTMY', 1)

            cost_MSE_val, cost_MSE_hist_val, _= sess.run([cost_MSE, cost_MSE_hist, optimal], feed_dict={C:C_train, E:E_train, CY:CY_train, Y: Y_train, BA: True })
...
for train_idx, test_idx in kf.split(Y_data[:-CELL_SIZE*2]):
    ...
    CY = tf.placeholder("float32", [CELL_SIZE, None, TIME_STAMP]) #cell_size, batch_size
    ...
    init()
    sess = tf.Session()
    cost_MAE, cost_MSE, cost_MAPE, optimal = model(C, E, CY, Y, BA)
    ...
    train(S_data, C_data,E_data, Y_data, cost_MAE, cost_MSE, cost_MAPE, cost_MAE_hist, cost_MSE_hist, cost_MAPE_hist, optimal, train_idx, test_idx, cr_idx, writer_train, writer_test, train_result, test_result,  CURRENT_POINT_DIR, start_from)
