...
def model_base(S, E, Y, DISCRIMINATOR_BA, DISCRIMINATOR_DR):
    ...
    loss_G = -tf.reduce_mean(Discriminator_model(layer, E[TIME_STAMP - 1], DISCRIMINATOR_BA, DISCRIMINATOR_DR, True)) + DISCRIMINATOR_ALPHA * train_MSE  # MSE 는 0~ t까지 있어봤자 같은 값이다.
    ...
    with tf.control_dependencies(D_update_ops):
        train_D = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE).minimize(loss_D, var_list=[vars_D]) #이 부분은 모델별로 고정
    ...
    return train_MSE,cost_MAE, cost_MSE, cost_MAPE, train_D, train_G, clip_D
...
def model_vector(S, E, Y, DISCRIMINATOR_BA, DISCRIMINATOR_DR):
    ...
    loss_D = -tf.reduce_mean(
        tf.log(Discriminator_model_Conv(Y, E[TIME_STAMP - 1], DISCRIMINATOR_BA, DISCRIMINATOR_DR)) + tf.log(
            1 - Discriminator_model_Conv(layer, E[TIME_STAMP - 1], DISCRIMINATOR_BA, DISCRIMINATOR_DR, True)))
    loss_G = -tf.reduce_mean(tf.log(Discriminator_model_Conv(layer, E[TIME_STAMP - 1], DISCRIMINATOR_BA,
...
def model(S, E, Y, DISCRIMINATOR_BA,  DISCRIMINATOR_DR):
    ...
    cost_MAE = MAE(Y, layer) #2차원
    cost_MSE = MSE(Y, layer)
    cost_MAPE = MAPE(Y, layer)
    adv_y = tf.concat([S, Y], axis=1) #원래 실험이 이럼 S는 12개로 그뒤에있는 24를 예측
    ...
    with tf.control_dependencies(update_ops):
        train_D = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE).minimize(loss_D)
        train_G = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE).minimize(loss_G)
    ...
    for tr_idx in range(start_from, TRAIN_NUM):
        ...
        for ba_idx in range(BATCH_NUM):
            ...
            if LATENT_VECTOR_FLAG:
                ...
                S_train = batch_slice(S_data, train_idx, ba_idx, 'FC')
...
if LATENT_VECTOR_FLAG:
    ...
    for train_idx, test_idx in Week_CrossValidation():
        ...
        train_MSE, cost_MAE, cost_MSE, cost_MAPE, train_D, train_G, clip_D = model_base(S, E, Y, DISCRIMINATOR_BA, DISCRIMINATOR_DR)
