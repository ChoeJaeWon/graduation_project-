...
def test(C_data, E_data, Y_data, cost_MAE, cost_MSE, cost_MAPE, cost_MAE_hist, cost_MSE_hist, cost_MAPE_hist, test_idx, tr_idx, global_step_te, cr_idx, writer_test, test_result):
    ...
    for ba_idx in range(BATCH_NUM):
        # Batch Slice
        C_test = batch_slice(C_data, test_idx, ba_idx, 'CONV', CELL_SIZE, len(test_idx))
        E_test = batch_slice(E_data, test_idx, ba_idx, 'LSTM', 1, len(test_idx))
        Y_test = batch_slice(Y_data, test_idx, ba_idx, 'LSTMY', 1, len(test_idx))
