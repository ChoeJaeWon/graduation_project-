...
def model(C, E, Y):
    ...
    adv_y= tf.concat([C[0], Y], axis=1)
    adv_g= tf.concat([C[0], layer], axis=1)
...
def train(C_data, E_data, Y_data, cost_MSE, train_D, train_G, train_idx):
    ...
    for tr_idx in range(TRAIN_NUM):
        ...
        for ba_idx in range(BATCH_NUM):
            ...
            _= sess.run([train_D], feed_dict={C:C_train, E:E_train, Y: Y_train, batch_prob: True, dropout_prob: FC_TR_KEEP_PROB, DISCRIMINATOR_BA:True, DISCRIMINATOR_DR: DISCRIMINATOR_TR_KEEP_PROB})
            cost_MSE_val, _ = sess.run([cost_MSE, train_G], feed_dict={C: C_train, E: E_train, Y: Y_train, batch_prob: True,dropout_prob: FC_TR_KEEP_PROB, DISCRIMINATOR_BA:True, DISCRIMINATOR_DR: DISCRIMINATOR_TR_KEEP_PROB})
...
def test(C_data, E_data, Y_data, cost_MAE, cost_MSE, cost_MAPE, test_idx, cr_idx):
    ...
    for ba_idx in range(BATCH_NUM):
        ...
        cost_MAE_val, cost_MSE_val, cost_MAPE_val = sess.run([cost_MAE, cost_MSE, cost_MAPE], feed_dict={C:C_test, E:E_test, Y:Y_test, batch_prob: False, dropout_prob: FC_TE_KEEP_PROB, DISCRIMINATOR_BA: False, DISCRIMINATOR_DR:DISCRIMINATOR_TE_KEEP_PROB})
        mae += cost_MAE_val
        mse += cost_MSE_val
        mape += cost_MAPE_val
